...
export default function Signup() {
  const domainList = ["@gmail.com", "@kakako.com", "@naver.com","@daum.net"];
  const [name , setname] = useState("");
  const [password , setpassword] = useState("");
  const [password2 , setpassword2] = useState("");
  const [email, setemail] = useState("");
  const [doamin, setdomain] = useState(domainList[0]);
  ...
  const signuphandler = async(e) => {
    e?.preventDefault();
      ...
      if (capchavale.status === 200){
        ...
        const sdata = await response.json();
          ...
          if (status === 200) {
            ...
            window.alert('보안을 위해 1분후 만료되면 리캡차를 다시 진행해 주세요 감사합니다.')
          }
      ...
      window.alert("리캡차를 다시 진행해 주세요 감사합니다.")
    }
  }
    ...
    return ( 
      <div >
          <form onSubmit={signuphandler} className="header_margin signup"> <div> <p style={{fontSize : '30px'}}> Sign up </p> <p> *은 필수입력</p></div>
            ...
            <input style={{ width : '150px'}} onChange={mail_change} type="text" placeholder="*이메일 *"/> 

              {/* <input className="tem"  type="text"  placeholder="이메일" value={doamin} style={ {width : '170px'}}/> */}

              
              <select  style = {{  width : '150px',height : '45.3px', border : 'none' , borderBottom : 'double' }}className="sel_domain" onChange={domainSelect} value={doamin}>
                {domainList.map((item) => (
            ...
            {email.indexOf('@') !== -1 ?  <p>이메일 도메인은 선택해주세요</p>: ""}
            ...
            <div style={{ display: 'flex',  width : '350px', marginLeft : '50px'}}>
              <input  onChange={password_change} type="password" placeholder="*패스워드 *" />
                {password !== "" && /^(?=.*[a-z])(?=.*[0-9])(?=.*[!@#$%^&*])(?=.{8,})/.test(password) && (
                  <Image src={check_img} width={40} height={30} />
                )}
             </div>
            ...
            <div style={ {display :'flex',  display: 'flex',  width : '350px', marginLeft : '50px'  }}>
              ...
              {password != "" && password2 != "" && password == password2 ? <Image  src = {check_img} width={40} height={30}/> : ""}  
            </div>
            ...
            <div>
              ...
              {email.indexOf('@') !== -1 ||  ! /^[가-힣]{1,5}$/.test(name) || password != password2 && password != "" && password2 != "" || email == "" || password2 == "" || isRecaptchachange === false ? (
